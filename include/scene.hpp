			...
			for (std::size_t shapeIndex = 0; shapeIndex < shapes.size(); ++shapeIndex) { // for each tinyobj shape
				...
				for (std::size_t triIndex = 0; triIndex < triCount; ++triIndex) { // for each triangle
					...
					for (auto& vertex : vertices) {
						...
						// Get vertex color
						vertex.color.x = attrib.colors[indices.vertex_index * 3 + 0] * material.diffuse[0] * (1.0f - material.metallic);
						vertex.color.y = attrib.colors[indices.vertex_index * 3 + 1] * material.diffuse[1] * (1.0f - material.metallic);
						vertex.color.z = attrib.colors[indices.vertex_index * 3 + 2] * material.diffuse[2] * (1.0f - material.metallic);
